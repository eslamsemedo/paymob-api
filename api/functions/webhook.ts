 ...
 */
export function redirectionHandler(req: Request, res: Response) {
  const secret = process.env.PAYMOB_HMAC!;
  if (!secret) return res.status(500).json({ ok: false, message: "Missing PAYMOB_HMAC" });

  // Normalize query (express can give string | string[])
  const qp: Record<string, any> = Object.fromEntries(
 ...
 */
export function processedWebhookHandler(req: Request, res: Response) {
  const secret = process.env.PAYMOB_HMAC!;
  if (!secret) return res.status(500).send("Missing PAYMOB_HMAC");
